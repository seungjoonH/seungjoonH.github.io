...
function CursorRingRoot(): ReactNode {
  ...
  useEffect(() => {
    ...
    const handleMove = (e: MouseEvent) => {
      ...
      setVisible(!!(target instanceof Element && target.closest(CURSOR_RING_SELECTOR)));
    };
    const handleLeave = () => setVisible(false);

    const syncVisibleFromPoint = () => {
      const { x, y } = posRef.current;
      setVisible(isOverTarget(x, y));
    };
  ...
  return (
    <div
      ref={ringRef}
      className={buildCls(styles.ring, visible && styles.visible)}
      aria-hidden="true"
    >
