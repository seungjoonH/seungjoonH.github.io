...
const projects = [
  {
    ...
    sections: [
      {
        title:
          '**드래그 가능 부동형 채팅 패널**, 위치 제어 커스텀 훅, Zustand Persist 좌표 복원',
        items: [
          '드래그, 화면 경계 보정, 포인터 이벤트를 커스텀 훅으로 분리하고, 컴포넌트는 렌더링만 담당하도록 역할 구분',
          '위치 변경 사유를 드래그, 리사이즈 보정, 패널 확장 보정으로 분류하고, 사유에 따라 Zustand Persist 저장 여부를 다르게 처리',
          '패널 확장 시 화면 밖으로 밀린 경우 보정 좌표를 저장하지 않아, 축소 후 원래 위치로 복원되는 UX를 유지',
        ],
        links: [{ type: 'doc', id: 'floating-component-spec' }],
      },
      {
        title:
          '**Playwright·socket.io 부하 실험** 기반 게임 입력 및 렌더링 최적화',
        items: [
          '입력 건마다 N명에게 브로드캐스트하는 구조에서 클라이언트 수신 이벤트가 O(N²)로 폭증할 수 있음을 확인',
          'Playwright와 socket.io-client로 10명 동시 입력 부하를 재현하고, FPS 로그와 Chrome Performance 프로파일링으로 병목을 측정',
          '클라이언트에서 100ms 단위로 입력을 묶어 전송하고, 서버에서 300ms 고정 주기로 점수와 랭킹을 전파하도록 구조 개선',
          '클라이언트 수신 이벤트 초당 **약 2,937건 → 32건**, 평균 FPS **30.7 → 60.5**로 개선, 브로드캐스트 복잡도 **O(N²) → O(N)**으로 개선',
        ],
        links: [{ type: 'doc', id: 'rendering-optimization-input-burst' }],
      },
      {
        title: '**NestJS WebSocket Gateway** 기반 실시간 채팅 구현',
        items: [
          '방 입퇴장 시 Socket.io Room에 join/leave하여 방 단위 브로드캐스트로 전역 전파 최소화',
          'JWT 핸드셰이크 인증 및 방 참여자 여부 검증 후 메시지 송신을 허용하고, 금칙어 필터링 및 Redis에 최근 30개 메시지 저장으로 재접속 시 복원',
          'Redis Adapter 적용으로 다중 인스턴스 환경에서도 Room 멤버십과 이벤트가 노드 간 동기화되도록 설계',
        ],
      },
      {
        title:
          '**Docker Compose** 로컬·프로덕션 환경 통일, GitHub Actions 빌드 파이프라인, NCP VPC 네트워크 분리',
        items: [
          'Docker Compose로 Server, Client, Redis, MySQL을 단일 구성으로 실행하고, 프로덕션에서는 MySQL을 Private Subnet으로 분리하여 운영',
          'GitHub Actions로 이미지를 빌드해 Docker Hub에 푸시하고, NCP VPC 내 프로덕션 서버에서 Pull하여 실행하는 배포 파이프라인 구성',
          'Web 계층은 Public Subnet에, MySQL은 Private Subnet에 분리 배치하여 데이터베이스를 외부에 직접 노출하지 않도록 구성',
        ],
  ...
  },
  {
    ...
    sections: [
      {
        title: '**접근성**: 시맨틱 구조 및 ARIA 설계',
        items: [
          '`header`, `nav`, `main`, `section`, `article` 등 **시맨틱 태그**로 페이지 구조를 잡고, 태그만으로 부족한 곳은 **ARIA 역할·이름·상태**로 보완. 모호한 안내 문구를 직접 검토해 고치고, **스크린 리더 안내와 포커스 순서**를 시각 UI와 맞춤',
          '`useConfigStore`로 **라이트/다크 모드** 상태를 전역 관리하고, 모드별 **CSS 변수 세트**로 **일관된 디자인 시스템**을 적용',
          '설정에 **Font Scale**을 두고 `clamp` / `cq`로 글자 배율을 키워도 **레이아웃이 유지**되도록 구성',
        ],
        links: [{ type: 'doc', id: 'portfolio-accessibility-design' }],
      },
      {
        title: '**사용자 경험**: 사용자 친화적 UI/UX',
        items: [
          '의도가 바로 보이지 않는 UI에는 **Tooltip**으로 다음 행동·설정 경로를 안내',
          '버튼·링크·입력에 Label과 보조 문구로 **역할을 한눈에** 알 수 있게 하고, `title`과 ARIA로 맥락을 보강',
        ],
      },
      {
        title: '**반응형 레이아웃**: 브레이크포인트 기반 대응',
        items: [
          '**mobile, tablet, desktop, wide** 네 구간을 두고, 그리드 열 수와 타이포를 맞춰 **어느 크기에서도 깨지지 않게** 설계',
          '`useResponsive` 훅으로 현재 구간을 추적해 **그리드 열 수·카드 안 태그/스택 표시 개수**에 반영',
        ],
        links: [{ type: 'doc', id: 'portfolio-responsive-design' }],
      },
      {
        title: '**프로젝트 검색**: 한글 자모 분리·초성 매칭',
        searchChip: { label: 'Extify', searchQuery: 'title:Extify show:all' },
        items: [
          '의 **한글 자모 분리·초성 매칭 알고리즘**을 JavaScript로 이식해 오타에 강한 검색 UX를 구현',
          '쿼리 정규화, 스택/태그 별칭 매핑, 결과 하이라이트를 쌓아 **찾기와 스캔**을 쉽게 만듦',
        ],
      },
      {
        title: '**다국어 지원**: react-i18next 리소스 구조',
        items: [
          '`react-i18next`로 UI 문자열을 로드하고, 프로젝트·경력 등 **도메인 데이터는 `ko`/`en` 디렉터리**로 나눠 동일 스키마를 유지',
          '키 네이밍과 **`{{variable}}` 보간**으로 동적 값을 넣고, 접근성 전용 문구는 **`a11y` 하위 트리**로 묶어 언어 전환 시 스크린 리더 문구도 함께 맞춤',
        ],
      },
      {
        title: '**인터랙션**: 애니메이션 기반 피드백',
        items: [
          '스크롤 **fade-in/out**으로 진행감을 주고, 히어로에 **parallax scrolling**을 적용',
          '헤더 Nav 버튼으로 각 섹션까지 **smooth scroll** 이동',
          '카드 **hover/focus Flip**과 모달 **전환 애니메이션**으로 마우스·키보드 모두에서 즉각 피드백',
        ],
      },
    ],
  },
