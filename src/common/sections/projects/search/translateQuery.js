...
const FALLBACK_PROJECTS_BY_LANG = { en: projectsEn, ko: projectsKo };
...
function getTranslationMaps(fromLang, toLang, dataByLang) {
  const tagMap = new Map();
  const stackMap = new Map();
  if (fromLang === toLang) return { tag: tagMap, stack: stackMap };

  const fromList = Array.isArray(dataByLang[fromLang]) ? dataByLang[fromLang] : [];
  const toList = Array.isArray(dataByLang[toLang]) ? dataByLang[toLang] : [];
 ...
 * @param {string} toLang
 * @param {{ en?: unknown[], ko?: unknown[] }} [projectsByLang] when omitted, uses bundled common project lists
 */
export function translateProjectSearchQuery(rawQuery, fromLang, toLang, projectsByLang = FALLBACK_PROJECTS_BY_LANG) {
