// 검색창 placeholder 타이핑·삭제 애니메이션 상태를 관리하는 훅
import { useEffect, useState } from 'react';
import { getJamoSequenceForTyping, jamosToDisplayForTyping } from './hangul';
...
type AnimationPhase = 'typing' | 'pauseFull' | 'deleting' | 'pauseEmpty';

function isJamo(c: string): boolean {
  if (typeof c !== 'string' || c.length !== 1) return false;
  const code = c.charCodeAt(0);
  return code >= JAMO_FIRST && code <= JAMO_LAST;
}

function prefixToDisplay(prefixList: string[]): string {
...
function getTypingSteps(str: string): string[] {
...
function getDeleteStepsBySyllable(str: string): string[] {
  const units = [...str];
  const steps: string[] = [];
...
export function usePlaceholderAnimation(examples: string[], opts: PlaceholderAnimationOptions = {}) {
