// 마크다운·KaTeX 문법을 React 노드로 렌더하는 리치텍스트 유틸
import { type ReactNode } from 'react';
import katex from 'katex';

export interface RichTextToken {
  type: 'text' | 'strong' | 'code' | 'math';
  value: string;
  displayMode?: boolean;
}

function renderKatex(latex: string, displayMode: boolean, key: string): ReactNode {
  try {
...
export function tokenizeRichText(s: string): RichTextToken[] {
...
function tokensToNodes(tokens: RichTextToken[], keyPrefix = ''): ReactNode[] {
...
export function renderLinkTitle(value: unknown, linkSepClassName?: string): ReactNode {
  const text = String(value ?? '');
